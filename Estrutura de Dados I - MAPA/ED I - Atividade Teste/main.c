...
//Implementeção do algoritmo de Dijkstra
void dijkstra(int vertices, int origem, int destino, int *custos){
	int i, v, cont = 0;
	int *ant, tmp;
	...
	/*Laço Principal*/
	do{
		/*Encontrando o vertice que deve estar em z */
		min = HUGE_VAL;
		for(i=0; i < vertices; i++){
			if(!z[i]){
		...
		/* Calculando a distância dos novos vizinhos de z */
...
void grafo_criar(void){
	do{
		printf("\nInforme o número de vertices (no minimo 3): ");
		scanf("%d", &vertices);
	} while (vertice < 3);
	if(!custos){
		free(custos);
